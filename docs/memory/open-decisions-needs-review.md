...
| OD-028 | project-map-runtime-entrypoints.md | lumos web komutu | `lumos web` / eksik `web/app.py` restore edilecek mi kaldırılacak mı? | medium | decision-draft | Karar taslağı: [`lumos-web-command-decision.md`](lumos-web-command-decision.md); kısmi netleşti — `lumos web` tanımlı ama `web/app.py` yok, komut kırık. Restore mi kaldırma mı seçimi ve uygulama paketi hâlâ needs-review. |
