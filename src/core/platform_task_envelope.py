...
IOS_LOGO_PRODUCTION_INPUT = "brand/source/logo001.png"
IOS_LOGO_BACKUP_INPUT = "web/static/brand/logo001.png"
# Target mark in lumos-core. The current Lumos producer does not consume it.
IOS_LOGO_TARGET_MARK = "ui/public/chat-lumos-mark.svg"
...
IOS_LOGO_ENVELOPE = PlatformTaskEnvelope(
    task_key="ios-logo",
    repo=IOS_LUMOS_REPO,
    scopes=IOS_LOGO_SCOPES,
    forbidden=(
        "App",
        "ios/App",
        "App/AI",
    ...
    packet=WorkPacket(
        ...
        kapsam=(
            f"PR-006 donmuş katalog; repo {IOS_LUMOS_REPO}. "
            f"Üretilen AppIcon: {IOS_LOGO_SCOPES[0]}; {IOS_LOGO_SCOPES[1]}. "
            f"Mevcut üretim girdisi: {IOS_LOGO_PRODUCTION_INPUT}. "
            f"Yedek: {IOS_LOGO_BACKUP_INPUT}. "
            f"Hedef işaret lumos-core {IOS_LOGO_TARGET_MARK} — mevcut üretici bunu "
            "henüz tüketmiyor; çalışan bağlantı değildir. "
            "apple-touch web varlığıdır; launch bu zarfta yok — ayrı platform anahtarı."
        ),
        dokunma=(
        ...
        kanit=(
            ...
            "write_local mevcut CU4 açık insan onayı kapısından geçer."
        ),
    ),
)
