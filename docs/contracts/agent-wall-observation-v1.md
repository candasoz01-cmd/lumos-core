...
- 2026-09-05 sandbox notu ("birincil sınır sandbox'tır; #832 sandbox MVP'siz merge adayı değildir") **geri getirilmedi**: fiilen merge edilen sınır aşağıda §1.1'de tanımlanan `allowed_roots` jail'idir ve `ADR-033` numarası bu arada başka bir karara (Account Activity Correlation) verildi. Gözlemcinin yürütme bağlamı mimarisi [ADR-034](../decisions/ADR-034-wall-observer-execution-context.md) ile karara bağlandı (**Accepted**, 2026-09-21; #872 · MERGED · `0c023e83`): birincil yön Seçenek 3 — git subprocess bağımlılığı kademeli kaldırılıp `.git/index` + doğrudan veri/stat okumaya geçilir; §1.1'deki jail geçiş boyunca ve kalan her git çağrısında defense-in-depth olarak kalır. O ADR kod izni değildir ve bu sözleşmenin bugünkü davranış tarifini değiştirmez; geçiş dilimleri kodu değiştirdikçe sözleşme buradan güncellenir.
