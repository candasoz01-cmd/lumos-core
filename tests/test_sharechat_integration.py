...
def test_sharechat_oauth_contract_uses_env_and_reports_identity_required(monkeypatch):
    for name in (
...
def test_sharechat_publish_requires_explicit_approval_and_never_fakes_execution():
    denied = _run("publish")
    ...
    assert approved.error == "sharechat_publish_connector_not_live"
