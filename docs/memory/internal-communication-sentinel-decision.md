# Sentinel katmanı ve iç iletişim protokolü — onaylı karar (OD-006 / OD-007)

> **Adlandırma (2026-07-23):** Eski dosya adı `internal-communication-bando-decision.md`. Katman adı **Sentinel** (legacy: Bando). Bkz. ADR-018 / OD-061 / [`legacy-naming.md`](./legacy-naming.md).
...
OD-006 (Sentinel katman varlığı) ve OD-007 (Lumos → iç katman iletişim protokolü) için **onaylı karar kaydı** tutmak.

Bu belge:

- Sentinel'in varsa hangi rolde kalacağını,
...
| Sentinel operasyonel runbook'u | Olay kaydı prosedürü OD-026 ile örtüşür; detay sonra. |
| Core / Local görev dağılımı detayı | Bu taslak Sentinel ve iç iletişim geçidine odaklanır. |
...
| 2 | Core, Local ve Sentinel **iç katmanlardır**; kullanıcıya ad veya arayüz olarak yansıtılmaz. | product-rules §2; internal-agent-layers §3 |
...
## 4. Sentinel rol kararı

**Karar (onaylı):** Sentinel, **yalnızca** güvenlik, gözlem ve anomali tespiti için ayrı bir katman olarak kabul edilir. Sıradan görev ajanı değildir; yürütme yapmaz; kullanıcıya görünmez; dış kaynaktan doğrudan komut, veri veya dosya kabul etmez.
...
| **İlişki** | Core (koordinasyon) ve Local (uygulama/yürütme) ile **rol karışmaz**; Sentinel iş yürütmez. |
...
**OD-006 durumu:** **decision-approved / implementation-pending.** Sentinel ayrı katman olarak onaylandı (rol ve sınır tanımı firm). Dağıtım modeli, edge-case senaryoları ve Core / Local sınır örnekleri uygulama detayı olarak bekliyor.

---

## 5. Sentinel'in yapamayacakları

Sentinel aşağıdaki işleri **yapmaz**; yapması veya dıştan buna yönlendirilmesi politika ihlalidir.

| # | Yasak | Gerekçe |
|---|--------|---------|
| 1 | Kullanıcı adına uygulama çalıştırma, komut yürütme, dosya yazma | Yürütme katmanı Local / Core alanı; Sentinel yürütme değildir. |
...
**Güvenlik olayı (firm):** Dış kaynaktan veya kullanıcı yüzeyinden Sentinel'a doğrudan ulaşma girişimi **güvenlik olayı (incident)** olarak sınıflandırılır; reddedilir ve kayıt altına alınması hedeflenir.
...
```
Dış dünya / kullanıcı → Lumos (geçit, orkestratör) → [doğrulanmış iç iletişim] → Core / Local / Sentinel
```
...
| 5 | Sentinel'a giden trafik de aynı geçit ve doğrulama ilkesine tabidir; istisna yoktur. |
...
| Dış kaynaktan Sentinel'a doğrudan erişim | Red + güvenlik olayı (incident) | Firm sınıflandırma |
| Sentinel tarafından tespit edilen anomali | Lumos'a iç rapor; kullanıcıya Sentinel adıyla değil, Lumos diliyle (gerekirse) | UX ve log formatı needs-review |
...
| Sentinel dağıtım modeli (süreç/ortam ayrımı) | OD-006 | Rol onaylı; deploy şekli uygulama detayı |
| Sentinel ↔ Core / Local sınır örnekleri (senaryo listesi) | OD-006 | Rol tablosu yeterli; edge case listesi uygulama detayı |
...
- Sentinel = güvenlik/gözlem/anomali katmanı; sıradan görev ajanı değil; yürütme yok; kullanıcıya görünmez.
- Dış → Sentinel doğrudan komut/veri/dosya kabulü yok; doğrudan erişim = güvenlik olayı.
...
|----|--------|------|----------------------------|
| **OD-006** | internal-agent-layers.md | Sentinel katman varlığı | **decision-approved / implementation-pending.** Ayrı katman; yalnızca güvenlik/gözlem/anomali; yürütme yok; kullanıcıya görünmez. Dağıtım modeli ve edge-case senaryoları uygulama detayı. |
