...
|----|--------------|------|-------------------|---------|--------|-----|
| OD-001 | security-architecture.md | Lumos Vault uygulaması | Secret'lar hangi vault/katman modelinde tutulacak; Lumos yüzeyinden nasıl ayrılacak? | high | decision-approved / implementation-pending | Karar: [`vault-secret-token-decision.md`](vault-secret-token-decision.md) — Lumos secret taşımaz; vault/kasa ayrı güvenli katman; Lumos yetkili geçit/orkestratör. **V1 teknoloji seçimi:** [`od-vault-v1-technology-selection.md`](od-vault-v1-technology-selection.md) — **harman + Infisical birincil** (OpenBao alternatif); sıfırdan reddedildi; PoC **private impl**. Yol haritası: [`vault-implementation-roadmap.md`](vault-implementation-roadmap.md). **Bekleyen:** Infisical PoC, depolama/dağıtım (V2), bridge adapter (**private**). |
...
| ID | Kaynak dosya | Konu | Kısa karar sorusu | Öncelik | Durum | Not |
|----|--------------|------|-------------------|---------|--------|-----|
| OD-031 | external-integrations-permissions.md | İletişim kanalları otomasyon modeli (mail ilk kanal) | Kullanıcı tanımlı iletişim kanalları takip/otomasyon modelinin kapsam, izin paketi ve onay seviyeleri ne? | medium | decision-approved / **implementation-partial** | Karar: [`mail-integration-approval-decision.md`](mail-integration-approval-decision.md) — tam ilke modeli. **Dar v1 pilot:** [`od-031-mail-dar-v1-scope.md`](od-031-mail-dar-v1-scope.md) — Gmail OAuth, `read`+`notify`; public stub `src/integrations/mail/`. **Bekleyen:** private Gmail OAuth, vault credential (Infisical), grant UI, sync, smoke. |
...
Son güncelleme: 2026-06-20 (vault/mail dar v1 design scope — docs-only)
