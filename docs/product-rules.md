...
Kod karşılığı bu dilimde **yoktur**: donmuş deterministik katalog modülü
(`platform_task_envelope`, checkout/parser'sız; eşleşmeyen cümle **STOP**)
ayrı bir dilimdir ve kendi kurucu onayını bekler — bu kayıt ona izin
değildir. ADR-004 router ve ADR-008 Agent Network değildir. Chat bağlama
(`api/bridge/chat.js`) bu dilimde yoktur. `Resources/Assets.xcassets/...` ve
`ios/Assets.xcassets/AppIcon.appiconset` uydurma/yanlış yoldur; katalogda yoktur.
