"""ADR-011 Faz 3: panel bridge keystore display honesty (keystore_ready vs consent proxy)."""
import json
import sys
...
def test_keystore_ready_follows_file_not_consent(tmp_path, monkeypatch):
    ...
    assert ks["consent_proxy_state"] == "onay kayıtlı"
...
def test_keystore_ready_true_when_keystore_file_exists(tmp_path, monkeypatch):
    ...
    assert ks["consent_proxy_state"] == "onay bekleniyor"


def test_guidance_lock_scope_consent_proxy(tmp_path, monkeypatch):
    monkeypatch.setenv("LUMOS_BASE_DIR", str(tmp_path))
    state = build_panel_read_state(repo_root=_REPO_ROOT)
    g = state["guidance"]
    assert g["lock_scope"] == "consent_proxy"
