...
|----|--------------|------|-------------------|---------|--------|-----|
| OD-001 | security-architecture.md | Lumos Vault uygulaması | Secret'lar hangi vault/katman modelinde tutulacak; Lumos yüzeyinden nasıl ayrılacak? | high | decision-approved / implementation-pending | Karar: [`vault-secret-token-decision.md`](vault-secret-token-decision.md) — Lumos secret taşımaz; vault/kasa ayrı güvenli katman; Lumos yetkili geçit/orkestratör. Yol haritası stub: [`vault-implementation-roadmap.md`](vault-implementation-roadmap.md). **Dar v1 keşif/tasarım:** [`od-vault-dar-v1-design.md`](od-vault-dar-v1-design.md) (teknoloji çerçevesi — seçim impl-pending). **Bekleyen:** somut vault ürünü/teknolojisi, depolama ve dağıtım modeli (**private impl**). |
