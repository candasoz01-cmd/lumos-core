...
_IOS_RE = re.compile(
    r"\b(ios|iphone|ipad|apple-touch-icon|appicon)\b",
    re.IGNORECASE,
)
_LOGO_RE = re.compile(
...
IOS_LOGO_ENVELOPE = PlatformTaskEnvelope(
    ...
    user_relevant=True,
    packet=WorkPacket(
        hedef="iOS ana işaretini OD-050 ChatLumos'a hizala",
        kapsam=(
            "Resources/Assets.xcassets AppIcon / apple-touch varlıkları; "
            "kaynak işaret lumos-core ui/public/chat-lumos-mark.svg"
        ),
        dokunma=(
            "App/ Swift, Network, AI conductor, lumos-core, açık PR #842-#850, "
            "main, commit/push/PR/merge/deploy"
        ),
        kanit=(
            "değişen dosya listesi, asset diff, splash/ikon görsel kanıt; "
