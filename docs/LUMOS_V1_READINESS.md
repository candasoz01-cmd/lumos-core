...
**Status:** Living document — reflects repo state as of 2026-06-11 (`65270f3` on `main`).  
...
- [ ] Görevler: add task → appears with `[Yerel]` → survives refresh — **FAIL (prod smoke):** `[Yerel]` rozeti görünür; fakat «Görev ekle» `TASKS_API_URL`=`http://127.0.0.1:8766` adresine POST atıyor, prod’da *Görev sunucusuna ulaşılamadı*; liste boş, yenileme sonrası kalıcılık doğrulanamadı (Playwright 2026-06-11).
...
- [x] `ruff check .` passes — `65270f3` üzerinde CI yeşil (ruff job dahil).
- [x] `pytest -q` passes — `65270f3` üzerinde CI yeşil (pytest job dahil).
- [x] GitHub Actions green on latest `main` commit — doğrulandı: `65270f3`.
...
- [x] PWA shell present (`8247a59` ancestor of release commit) — doğrulandı: `git merge-base --is-ancestor 8247a59 HEAD` (65270f3); prod manifest geçerli (yukarıdaki manifest maddesi) (2026-06-11 smoke).
