...
| OD-008 | project-workflow.md | Continuous progress vs tek-adım | `docs/workflow-rules.md` continuous progress ile `.cursor/rules/tek-adim-ilerleme.mdc` hangisi öncelikli? | high | needs-review | Karar taslağı: [`workflow-decision-alignment.md`](workflow-decision-alignment.md); kısmi netleşti — tek hedef üstün; canonical öncelik belgesi uygulama kararı bekliyor. Migration: örtüşme / çelişki riski |
| OD-009 | project-workflow.md | Agent-first canonical kaynak | Agent-first execution kuralı tek canonical yerde mi tutulacak? | high | needs-review | Karar taslağı: [`workflow-decision-alignment.md`](workflow-decision-alignment.md); davranış net; çift kayıt birleştirme uygulama kararı bekliyor. `.cursor/rules/agent-calisma-kurallari.mdc` ile çift kayıt |
| OD-010 | project-workflow.md | CI tamamlanma kriteri | CI yeşil olmadan tamamlandı sayma kuralı workflow belgeleriyle tam hizalı mı? | high | needs-review | Karar taslağı: [`workflow-decision-alignment.md`](workflow-decision-alignment.md); kısmi netleşti — firm bitti kriterleri; dosya senkronu uygulama kararı bekliyor. §5; `kando-lumos-multi-agent.mdc` referansı |
