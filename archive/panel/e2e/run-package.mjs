/**
 * Resmi paket kapısı (local/demo): LUMOS_PANEL_TASKS_API_BASE=false.
 */
...
async function run() {
  ...
  try {
    ...
    if (tasksAfterCreate.indexOf(MARK) === -1) fail("after-create/tasks-list", "oluşturma sonrası görev listesinde başlık yok");
    ...
    if (completedView.indexOf(MARK) === -1) fail("after-complete/completed-filter", "tamamlandı filtresinde başlık yok");
    ...
    var silOk = await page.evaluate(function () {
      ...
      if (txt.indexOf("bulunamadı") !== -1) return { ok: false, reason: "sil yanıtı başarısız: " + txt.slice(0, 100) };
      if (txt.indexOf("silindi") === -1) return { ok: false, reason: "silindi metni yok: " + txt.slice(0, 100) };
      return { ok: true };
    });
    if (!silOk.ok) fail("after-delete/chat-reply", silOk.reason || "sil chat");

    await flow.assertDeletedHiddenAllTaskFilters(page, "after-delete/tasks-filters");
    await flow.assertTrashVisible(page, "after-delete/trash");
    await flow.assertLogsFullChain(page, "after-delete/logs");
    await flow.assertDashboardChain(page, "after-delete/dashboard");
    ...
    await flow.assertPostReload(page, "post-reload");
  } finally {
    await browser.close();
    await new Promise(function (r) {
      server.close(function () {
...
run()
  .then(function () {
    console.log("PACKAGE_E2E_RESULT: PASS");
    process.exit(0);
  })
  .catch(function (err) {
    console.error("PACKAGE_E2E_RESULT: FAIL");
