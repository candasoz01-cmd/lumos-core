...
| OD-006 | internal-agent-layers.md | Sentinel katman varlığı | Sentinel ayrı katman olarak kalacak mı; görev ve yetki sınırları net mi? | high | decision-approved / implementation-pending | Karar: [`internal-communication-sentinel-decision.md`](internal-communication-sentinel-decision.md) — Sentinel yalnızca güvenlik/gözlem/anomali için ayrı katman; sıradan görev ajanı değil; yürütme yok; kullanıcıya görünmez; dış kaynaktan doğrudan komut/veri/dosya kabul etmez. **Bekleyen:** dağıtım modeli, edge-case senaryoları. |
| OD-007 | internal-agent-layers.md | İç iletişim protokolü | Lumos→iç katman imzalama/şifreleme protokolü ve anahtar döngüsü ne? | high | decision-approved / implementation-pending | Karar: [`internal-communication-sentinel-decision.md`](internal-communication-sentinel-decision.md) — Lumos→iç katman iletişimi doğrulanmalı; imzalama ve/veya şifreleme tercih edilir. **Bekleyen:** protokol, mesaj formatı, anahtar döngüsü, vault entegrasyonu — public repoda tanımlanmaz; private/gizli uygulama paketini bekler. |
...
| — | ADR-006 / ADR-011 / enforcement map | Trust Faz 4 — merkezi trust tüketimi | `keystore_ready` ve `session_unlocked` ayrı alanlar merkezi trust modelinde ne zaman birleşir? | high | **needs-review (ONAY GEREKİYOR)** | ADR-011 Faz 1–3 **kapandı** (#436–#438); Panel LockState env bypass'ı kaldırıldı ve runtime snapshot API fail-closed çalışır; Faz 4 merkezi trust motor + panel process snapshot besleme **açık** — bkz. [ADR-012 açık maddeler](../decisions/ADR-012-lumos-security-codex.md#açık-kalan-maddeler-codex-kapanış-öncesi) |
| — | ADR-012 / CU4 skeleton | Köprü `consume_confirmation` wiring | PR-C6 shadow adapter sonrası köprü yürütmede confirmation consume ne zaman bağlanır? | high | **closed** | **Kapandı (2026-06-21):** Wave 1 Seçenek B — köprü approve/resume `consume_confirmation` + opt-in env (#494–#495); karakterizasyon #491–#493; RB-02 kapanış koşulu sağlandı |
...
| — | ADR-012 / P2 analiz | P2 `SECURITY_NEVER_AUTO` tam küme eşlemesi | Dört üyenin tüm silme/yazma yollarında engine/policy eşlemesi tamamlanacak mı? | high | **closed** | **Kapandı (2026-06-21):** Wave 1 Seçenek B — tam eşleme tablosu #497, engine + panel/CLI/store sync #498; karakterizasyon #496; RB-04 kapanış koşulu sağlandı |
...
| — | ADR-010 / ADR-012 / enforcement map | Faz-2 enforcement dalgası (#460–#464) | E2E, varsayılan-on, PR-C6, P2 engine docs hizası tamam mı? | medium | **closed (docs)** | **Kapandı (2026-06-21):** E2E #460; opt-in #461 (DL-C18); PR-C6 kısmi #462; P2 dar merge #463; milestone docs sync #464; ADR-010 + chain-summary + open-decisions senkronu (DL-C19). **Wave 1 (2026-06-21):** PR-C6 wiring #491–#495 ve P2 tam eşleme #496–#498 merge. **Sonraki kısmi:** Panel LockState env bypass kapandı + runtime snapshot API. **Açık enforcement satırları:** Trust Faz 4 merkezi motor, sensitivity chain, panel process snapshot besleme. |
| — | ADR-013 / ADR-001 | Quantum Readiness Faz-2 MVP | Panel `GET /quantum-readiness` ve yerel tarayıcı uygulandı mı? | medium | **closed (kısmi — docs)** | **Kapandı (2026-06-21):** yerel tarayıcı #468; panel GET + UI fetch #469; ADR-001/013 sync #470; panel/landing copy #471–#473; DL + open-decisions #472; ADR-001 iskelet cross-ref + chain-summary #474; panel noLiveScan fetch copy #475; milestone docs sync #476; ADR-013 fetch vs mock wording #477 (DL-A25, DL-C20); CLI `lumos quantum-readiness` #479; panel live fields UI #480; docs sync #481; panel migration tables #482; docs sync #483; checklist Faz-2 closure; `hazırlık_raporu` display-only badge (panel UI, ADR-013 türetim tablosu). Eski «endpoint yok» / «Faz-2 onay bekliyor» / «CLI alt komutu açık» / «tam panel alan seti açık» / «hazırlık_raporu badge ertelendi» maddeleri geçersiz. Entropy davranışı değiştirilmez. |
...
| ID | Kaynak dosya | Konu | Kısa karar sorusu | Öncelik | Durum | Not |
|----|--------------|------|-------------------|---------|--------|-----|
| OD-060 | ADR-017 / lumos-institutional-service-foundation.md | Ticari birimler, Lumos Dünya ve ülke entegrasyonu | Lumos Bank, Lumos Sepet, Lumos POS, Lumos Dünya ve Ülke Sistemleri Entegrasyon Katmanı hangi güven sözleşmelerini paylaşır; hangi lisans, veri ve yetki kapılarıyla ayrılır? | high | decision-approved / implementation-pending | Foundation kararı: [`ADR-017`](../decisions/ADR-017-regulated-service-entity-boundaries.md). `candasoz01-cmd/Lumos` #102/#103 hizası: **Lumos Devlet kaldırıldı**, Lumos Dünya küresel yüzey; ülke bağlantısı markasız teknik katman. Entegrasyon varsayılanı: mevcut sistem authoritative, yetki `deny`, önce envanter + salt-okuma adaptörü; yazma/müdahale yalnız ülkenin sözleşmeli talebi ve operasyon bazlı matrisiyle. Kanıt skoru ve ülke paketi: [`lumos-institutional-service-foundation.md`](../analysis/lumos-institutional-service-foundation.md). **Bekleyen:** ilk ülke/sistem envanteri, veri sınıflandırma, threat model, partner ve pilot paketi. Gerçek ödeme/banka/PSP/ülke sistemi bağlantısı yok; OD-011 üst sınırı sürer. |
| OD-061 | internal-agent-layers.md / legacy-naming.md | Legacy katman adları | Legacy iç katman adları emekli; Core/Local/Sentinel mi? | high | **decision-approved** | Karar: [`ADR-018`](../decisions/ADR-018-internal-layers-core-local-sentinel.md) · [`od-061-legacy-layer-naming-retirement.md`](od-061-legacy-layer-naming-retirement.md) · [`legacy-naming.md`](legacy-naming.md). Rol sınırları OD-006/007 değişmez. **Bekleyen (ayrı):** teknik tanımlayıcı cutover; `KandoLumos` marka kararı. |
| OD-062 | ADR-019 / ADR-004 | Ürün yüzü ayrımı ve ModelRegistry | Son kullanıcı yüzeyi ile iç operatör yüzeyi (Lumos Agent Wall, eski ad: Command Wall) nasıl ayrılır; model/provider seçimi Router'ın altında hangi katmanda durur? | high | **decision-approved / implementation-pending** | Karar: [`ADR-019`](../decisions/ADR-019-product-surface-separation-modelregistry.md) · [`ADR-004`](../decisions/ADR-004-ai-router-routing-layer.md) § Router altında ModelRegistry sınırı. Hedef: `Lumos User Surface → Router → ModelRegistry → Providers`, ayrı gözlem yüzeyi `Lumos Agent Wall → Router + Agent/Session/Task state`. `ModelRegistry` ≠ `IntegrationRegistry` normatif. Kullanıcı yalnız Lumos görür (PR-005). ADR-008 öncelik sırası (Firewall → Trust → Router → Memory → Agent Network) değişmedi. **Bekleyen:** `ModelRegistry` arayüz sözleşmesi; Router model seçim sinyalleri; Lumos Agent Wall operatör yüzeyi teknik paketi + yetkilendirme; kullanıcı yüzü sızıntı guard testi — kapsam kabul kriteri ADR-019 § Bekleyen'de tanımlı (`provider`, `agent_id`, `instance_id`, `session_id`, `workspace_path`, heartbeat, PR/merge gate; public API yanıtları dahil); FAZ-1 sonrası provider evaluation paketi (Kimi/Moonshot backlog'da). **Kod yazılmadı; STOP LIST sürüyor.** Decision Queue alt kavramı → **OD-063**. |
| OD-063 | ADR-019 | Agent Wall → Decision Queue | Paralel ajan oturumlarında insan onayı bekleyen karar noktaları Agent Wall içinde mi görünür; ayrı ürün mü? | high | **decision-approved / implementation-authorized (minimal, 2026-08-25)** | 2026-08-24 kurucu: Agent Wall içinde Decision Queue / Human Action Queue. Amaç: “hangi süreç, hangi karar noktasında, kimin aksiyonunu bekliyor?” İç operatör görünürlüğü. **Güncelleme (2026-08-25, kurucu):** minimal human-on-exception dilimi **yetkilendirildi** — kapsam: agent-status şema v2 (`blocked` / `awaiting_decision` + `wait_reason` ayrımı), salt-okunur Decision Queue görünümü, güvenilir-yazıcı sınırı (ayrı onay). Saha kanıtı: ADR-008 § Gözlem (2026-08-24; PR #802). **Korunan sınırlar:** yeni ürün yok, yeni Agent Network yok, ajanlar arası doğrudan komut yok, auto-merge / auto-deploy / dış gönderim / geri döndürülemez eylem yok; kapsam mevcut `src/lumos_board/` dilimlerinin genişletmesidir, STOP LIST'i delmez. **Değil:** yeni ROADMAP maddesi, `Lumos Workboard` / `Control Wall` ürünü, yeni ADR. Kaynak: [`ADR-019`](../decisions/ADR-019-product-surface-separation-modelregistry.md). |

---

Son güncelleme: 2026-08-25 (OD-063 — minimal human-on-exception dilimi yetkilendirildi; sınırlar korunuyor)
