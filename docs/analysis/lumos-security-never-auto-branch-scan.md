...
| **P1** | Panel `POST /tasks/restore` policy gate yok | `panel_tasks_server.py` `_post_restore` | **Kapandı** — `fix/panel-restore-gate` |
...
**P2:** TaskEngine `run_task` içinde `SECURITY_NEVER_AUTO` ayrı branch (ADR-006 bilinçli takip).
