...
| Alan | Değer |
| --- | --- |
| Durum | SÖZLEŞME — bu PR yalnız sözleşme ve kabul kriterleri; **ürün kodu yok** |
| Kapsam | Panel/dashboard kartlarının sağlık durumu: durum sözlüğü, freshness, backend→UI türetme, UI semantiği |
...
iddiasında bulunuyor ama bu iddianın arkasında ölçüm yok. Sözleşme, ölçüm ile
gösterge arasındaki bağı tanımlar.
...
1. **Aynı backend state → aynı UI semantiği.** Aynı `state` iki farklı kartta
   aynı rozeti, aynı renk rolünü ve aynı metin şablonunu üretir.
2. **Farklı state'ler görsel olarak ezilmez.** Beş durumun beş ayrı görsel
   kimliği vardır; hiçbir ikisi tek göstergeye indirgenmez.
3. **`unknown ≠ failed ≠ not_configured`.** Üçü tek "sorunlu" kovasına
   toplanmaz.
4. **`null` freshness gerçek bir durumdur.** Gizlenmez, `now()` ile
   doldurulmaz, göreli zamana çevrilmez.
5. **Global oturum/bulut durumu kartlardan hesaplanmaz.** Tek yön: başlık kendi
...
9. Sözleşme uygulanmadan hiçbir rozet "canlı" iddiasında bulunmaz; bugünkü 18
...
- **Beklenen görünür sonuç:** sözleşme uygulandığında panel bugünkünden **daha
  ...
  Bu bir gerileme değil — bugünkü yeşil zaten ölçüme dayanmıyor. Sözleşmenin
  amacı paneli iyi göstermek değil, **doğru** göstermek.
