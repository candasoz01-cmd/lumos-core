...
|---|--------------|-----------|-----------|-------------|------------|----------------|---------------------|--------------|
| 1 | PR-C6 wiring kapsamı | Shadow adapter; legacy `approval_token` yürütme | `consume_confirmation` + opt-in env ile köprü birleşimi | A: geriye uyum, düşük kırılma — B: tek onay yolu, CU4 hizası | A: duplicate onay, kısmi codex — B: migration, opt-in etkisizlik | `lumos_gate`, `task_dispatch`, `kando_bridge/server`, `confirmation_policy`, `cursor_bridge` | A: düşük — B geri alım: orta–yüksek | **kapandı (Seçenek B, Wave 1 #491–#495)** |
| 2 | P2 genişletme sınırı | Dar engine branch (#463) | Tam action/kind eşleme tablosu | A: minimal diff — B: küme tam kapsama, drift azalması | A: tag bypass — B: false positive, matris bakımı | `profiles.py`, `engine.py`, `action_policy`, `action_registry`, panel/CLI | A: düşük — B geri alım: orta–yüksek | **kapandı (Seçenek B, Wave 1 #496–#498)** |
