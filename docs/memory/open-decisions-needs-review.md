...
| — | ADR-013 / ADR-001 | Quantum Readiness Faz-2 MVP | Panel `GET /quantum-readiness` ve yerel tarayıcı uygulandı mı? | medium | **closed (kısmi — docs)** | **Kapandı (2026-06-21):** yerel tarayıcı #468; panel GET + UI fetch #469; ADR-001/013 sync #470; panel/landing copy #471 (DL-A25, DL-C20). Eski «endpoint yok» / «Faz-2 onay bekliyor» maddeleri geçersiz. **Açık (onay gerekir):** Lumos CLI alt komutu; tam panel alan seti. Entropy davranışı değiştirilmez. |
...
Son güncelleme: 2026-06-21 (Quantum Readiness Faz-2 milestone — DL-A25/DL-C20; #468–#471; «endpoint yok» kapandı)
