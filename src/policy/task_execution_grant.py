...
Ajan anahtar üretmez; kullanıcıdan da istenmez. Görev kabul edilince merkezi
Task Authority üretir.

    Kimlik → oturum → görev ID → verilen yetki → istenen işlem
        → policy kararı → (gerekirse onay) → yürütme sonucu
...
SCHEMA_GRANT = "lumos.task_execution_grant.v1"
...
LEDGER_REL = Path("ledgers") / "execution_ledger.jsonl"
...
# Back-compat aliases for earlier kernel tests / docs
EVENT_ISSUED_LEGACY = EVENT_ISSUED
AUDIT_REL = LEDGER_REL
...
def binding_from_mapping(
    ...
    *,
    ...
    """Gate bundle → bağ; kimlik yoksa None (fail-closed)."""
...
def _base(base_dir: Path | str | None) -> Path:
    return Path(base_dir).resolve() if base_dir is not None else lumos_base_dir()
...
# ---------------------------------------------------------------------------
# 1. Task Registry — resmi görev kaydı (kapının bir yarısı)
# ---------------------------------------------------------------------------
...
def load_registered_task(task_id: str, *, base_dir: Path | str | None = None) -> dict[str, Any] | None:
    try:
        path = _registry_path(_base(base_dir), task_id)
    except ValueError:
        return None
...
def _register_task(binding: ExecutionBinding, *, base: Path) -> dict[str, Any]:
...
def _attach_grant_to_registry(
    ...
    *,
    ...
    record = _load_json(path)
    if record is None:
        raise ValueError(REASON_UNKNOWN_TASK)
    record["grant_id"] = grant_id
    record["token_hash"] = token_digest
    _atomic_write_json(path, record)


# ---------------------------------------------------------------------------
# 3. Immutable Audit Ledger — kanıt; allow/deny kaynağı değil
# ---------------------------------------------------------------------------
...
def _tip_path(base: Path) -> Path:
    return base / LEDGER_TIP_REL
...
def append_ledger_entry(
    ...
    *,
    ...
    """Append-only hash-zinciri. Bu fonksiyon yetki vermez."""
    prev = _read_tip(base)
    body = {
...
def verify_ledger_chain(base_dir: Path | str | None = None) -> bool:
    """Defter bütünlüğü. Sonucu kapı olarak kullanılmaz."""
    prev = GENESIS_HASH
...
# ---------------------------------------------------------------------------
# 2. Capability Token — Task Authority mint; executor consume
# ---------------------------------------------------------------------------
...
def _grant_path(base: Path, grant_id: str) -> Path:
    ...
    return base / GRANTS_DIR / f"{grant_id}.json"
...
def accept_execution_task(
    ...
    *,
    ...
    """
    Merkezi Task Authority: görevi kaydeder ve yetki üretir.

    Ajan ve kullanıcı bu fonksiyonun yerine anahtar üretmez / girmez.
    """
    ...
    base = _base(base_dir)
    _register_task(binding, base=base)
    issued = _mint_capability(binding, base=base, ttl_seconds=ttl)
    _attach_grant_to_registry(
        binding.task_id,
        base=base,
        grant_id=issued.grant_id,
        token_digest=issued.token_hash,
    )
    append_grant_audit(
        base,
        EVENT_ACCEPTED,
...
def issue_task_execution_grant(
    ...
    *,
    ...
    """Yalnız kayıtlı görev için. Yeni görev kabulü `accept_execution_task`."""
...
def _consume_locked(
    ...
    *,
    ...
    failed = _validate_capability(record, token=token, expected=expected)
