...
(function () {
  ...
  // ——— Contract (ekran veri şemaları; backend entegrasyonunda referans) ———
  // Her ekran için beklenen alanlar. Stub ve adapter bu şekle uyar.
  var CONTRACTS = {
    dashboard: { title: "", subtitle: "", metrics: [], sections: [{ title: "", events: [], warnings: [], links: false }] },
    tasks: { title: "", subtitle: "", filters: [], activeFilter: "", listItems: [], selectedId: null, selectedTask: null, emptyListTitle: "", emptyListDesc: "", detailTitle: "", runNoteTitle: "", runNoteBody: "" },
    ...
    trash: { title: "", subtitle: "", summaryMetrics: [], listItems: [], selectedId: null, selectedItem: null, detailTitle: "", emptyListTitle: "", emptyListDesc: "", emptyDetailPlaceholder: "" },
    logs: { title: "", subtitle: "", filters: [], activeFilter: "", events: [], sectionTitle: "" },
    system: { title: "", subtitle: "", healthCards: [{ title: "", status: "", note: "" }] },
  };

  // ——— Stub üreticileri (contract'a uyan; state -> ekran verisi) ———
  ...
  // ——— Normalize / hafif doğrulama (eksik alanlarda güvenli varsayılan) ———
  function normalizeDashboard(data) {
    if (!data) return buildDashboardStub(mockState);
  ...
  function normalizeTasks(data) {
    if (!data) return buildTasksStub(mockState);
    ...
    data.selectedTask = data.selectedTask || null;
    return data;
  }

  function normalizeSandbox(data) {
    if (!data) return buildSandboxStub(mockState);
    data.metrics = Array.isArray(data.metrics) ? data.metrics : [];
    data.sections = Array.isArray(data.sections) ? data.sections : [];
    return data;
  }
  ...
  function normalizeConfig(data) {
    if (!data) return buildConfigStub(mockState);
    data.metrics = Array.isArray(data.metrics) ? data.metrics : [];
    data.sections = Array.isArray(data.sections) ? data.sections : [];
    return data;
  }
  ...
  function normalizeIdentity(data) {
    if (!data) return buildIdentityStub(mockState);
    data.metrics = Array.isArray(data.metrics) ? data.metrics : [];
    data.sections = Array.isArray(data.sections) ? data.sections : [];
    return data;
  }
  ...
  function normalizeKeystore(data) {
    if (!data) return buildKeystoreStub(mockState);
    data.metrics = Array.isArray(data.metrics) ? data.metrics : [];
    data.sections = Array.isArray(data.sections) ? data.sections : [];
    return data;
  }

  function normalizeTrash(data) {
    if (!data) return buildTrashStub(mockState);
    data.summaryMetrics = Array.isArray(data.summaryMetrics) ? data.summaryMetrics : [];
    data.listItems = Array.isArray(data.listItems) ? data.listItems : [];
    data.selectedItem = data.selectedItem || null;
    return data;
  }

  function normalizeLogs(data) {
    if (!data) return buildLogsStub(mockState);
  ...
  function normalizeSystem(data) {
    if (!data) return buildSystemStub(mockState);
  ...
  // ——— Adapter (contract'a hizalı; kaynak: stub + normalizer; yarın backend için sadece bu katmanda mapping değişir) ———
  function getDashboardData() {
    return normalizeDashboard(buildDashboardStub(getEffectiveState()));
  }

  function getTasksData() {
    return normalizeTasks(buildTasksStub(getEffectiveState()));
  }

  function getSandboxData() {
    return normalizeSandbox(buildSandboxStub(getEffectiveState()));
  }

  function getConfigData() {
    return normalizeConfig(buildConfigStub(getEffectiveState()));
  }

  function getIdentityData() {
    return normalizeIdentity(buildIdentityStub(getEffectiveState()));
  }

  function getKeystoreData() {
    return normalizeKeystore(buildKeystoreStub(getEffectiveState()));
  }

  function getTrashData() {
    return normalizeTrash(buildTrashStub(getEffectiveState()));
  }

  function getLogsData() {
    return normalizeLogs(buildLogsStub(getEffectiveState()));
  }

  function getSystemStatusData() {
    return normalizeSystem(buildSystemStub(getEffectiveState()));
  }
