...
| OD-028 | project-map-runtime-entrypoints.md | lumos web komutu | `lumos web` / eksik `web/app.py` — B1: alt komutu kaldır (restore değil) | medium | decision-approved | Karar: [`lumos-web-command-decision.md`](lumos-web-command-decision.md) — **B1** seçildi; `web/app.py` restore edilmeyecek. **Uygulama bekliyor** (§12.2): `__main__.py`, `pyproject.toml`, test, mimari belge senkronu. **Closed değil** — kod henüz değişmedi. |
