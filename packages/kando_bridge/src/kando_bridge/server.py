...
try:
    ...
    def attach_execution_dispatch_to_out(out: dict, *, repo_root: Path | None = None) -> None:
        """Stub: kando_runtime yoksa yürütme etiketi eklenmez."""
        return
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def _send_json_head(self, status: int, payload: dict) -> None:
    ...
    def do_HEAD(self) -> None:
        if not self._check_loopback():
            return
        req_path = _normalize_request_path(urlparse(self.path).path)
        if req_path == "/health":
            self._send_json_head(
                200,
                {
                    "status": "ok",
                    "gate": True,
            ...
            return
        if req_path == "/":
            self._send_json_head(200, {"ok": True, "service": "kando_bridge_server"})
            return
        self.send_error(404)
    ...
    def _handle_chat(self) -> None:
        ...
        if classify_bridge_message_intent(message) == "task":
            try:
                from core.user_intent_classifier import classify_user_message_intent
                # run_task / extract_task: lumos-core/src/core/simple_chat_task.py
