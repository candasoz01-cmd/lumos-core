...
| — | ADR-013 / ADR-001 | Quantum Readiness Faz-2 MVP | Panel `GET /quantum-readiness` ve yerel tarayıcı uygulandı mı? | medium | **closed (kısmi — docs)** | **Kapandı (2026-06-21):** yerel tarayıcı #468; panel GET + UI fetch #469; ADR-001/013 sync #470; panel/landing copy #471–#473; DL + open-decisions #472; ADR-001 iskelet cross-ref + chain-summary #474; panel noLiveScan fetch copy #475; milestone docs sync #476; ADR-013 fetch vs mock wording #477 (DL-A25, DL-C20); CLI `lumos quantum-readiness` #479; panel live fields UI #480; panel migration tables #482 (`long_lived_data`, `hard_to_change_deps`, `prioritized_migration_plan`). Eski «endpoint yok» / «Faz-2 onay bekliyor» / «CLI alt komutu açık» / «tam panel alan seti açık» maddeleri geçersiz. **Ertelendi (opsiyonel):** `hazırlık_raporu` genel durum badge'i (`tamamlandi`/`kısmi`/`doğrulanamadi`) — spesifikasyon § Panel alanları; Faz-2 panel kapsamı docs-kapalı sayılır. Entropy davranışı değiştirilmez. |
...
Son güncelleme: 2026-06-21 (Quantum Readiness Faz-2 milestone — DL-A25, DL-C20; #468–#482; panel alan seti docs-kapalı; `hazırlık_raporu` badge ertelendi)
