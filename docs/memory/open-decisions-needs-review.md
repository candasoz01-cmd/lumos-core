...
| ID | Kaynak dosya | Konu | Kısa karar sorusu | Öncelik | Durum | Not |
|----|--------------|------|-------------------|---------|--------|-----|
| OD-060 | ADR-015 / lumos-institutional-service-foundation.md | Düzenlemeye tabi hizmet kuruluşları | Lumos Bank, Lumos Sepet, Lumos POS ve Lumos Devlet hangi ortak güven sözleşmelerini paylaşır; hangi lisans, veri, yetki ve ülke kapılarıyla ayrılır? | high | decision-approved / implementation-pending | Foundation kararı: [`ADR-015`](../decisions/ADR-015-regulated-service-entity-boundaries.md). Lumos Devlet varsayılanı: mevcut sistem authoritative, yetki `deny`, önce envanter + salt-okuma adaptörü; yazma/müdahale yalnız ülkenin sözleşmeli talebi ve operasyon bazlı matrisiyle. Kanıt skoru ve ülke paketi: [`lumos-institutional-service-foundation.md`](../analysis/lumos-institutional-service-foundation.md). **Bekleyen:** hukuk/marka incelemesi, tüzel/lisans modeli, ilk ülke/sistem envanteri, veri sınıflandırma, threat model, partner ve pilot paketi. Gerçek ödeme/banka/PSP/kamu bağlantısı yok; OD-011 üst sınırı sürer. |
