...
| OD-063 | ADR-019 | Agent Wall → Decision Queue | Paralel ajan oturumlarında insan onayı bekleyen karar noktaları Agent Wall içinde mi görünür; ayrı ürün mü? | high | **decision-approved / implementation-deferred** | 2026-08-24 kurucu: Agent Wall içinde Decision Queue / Human Action Queue. Amaç: “hangi süreç, hangi karar noktasında, kimin aksiyonunu bekliyor?” İç operatör görünürlüğü. **Değil:** yeni ROADMAP maddesi, `Lumos Workboard` / `Control Wall` ürünü, yeni ADR, kod, UI. Meet tek-ses provası (`cursor/meet-single-voice-2688`) bölünmez. Uygulama kritik akışlardan sonra. Kaynak: [`ADR-019`](../decisions/ADR-019-product-surface-separation-modelregistry.md). |

---

Son güncelleme: 2026-08-24 (OD-063 Agent Wall Decision Queue — fikir kaydı; kod/UI yok)
