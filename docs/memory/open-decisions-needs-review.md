...
|----|--------------|------|-------------------|---------|--------|-----|
| OD-001 | security-architecture.md | Lumos Vault uygulaması | Secret'lar hangi vault/katman modelinde tutulacak; Lumos yüzeyinden nasıl ayrılacak? | high | decision-draft | Karar taslağı: [`vault-secret-token-decision.md`](vault-secret-token-decision.md); kısmi netleşti — Lumos secret taşımaz, vault ayrı katman olur. Somut vault ürünü/depolama/dağıtım modeli hâlâ needs-review. |
| OD-002 | security-architecture.md | Token / vault entegrasyonu | Token ve credential yönetimi vault + bridge ile nasıl birleşecek? | high | decision-draft | Karar taslağı: [`vault-secret-token-decision.md`](vault-secret-token-decision.md); kısmi netleşti — token/credential yüzeyde açık tutulmaz, bridge kontrollü geçit olur. Token formatı, şema ve endpoint entegrasyonu hâlâ needs-review. |
| OD-003 | data-vault-user-data.md | Vault amaç bazlı erişim | Vault Lumos'a hangi kapsamda, hangi amaç kodlarıyla erişim verecek? | high | decision-draft | Karar taslağı: [`vault-secret-token-decision.md`](vault-secret-token-decision.md); kısmi netleşti — erişim sınırlı, amaçlı, onaylı ve görünür olmalı. Amaç kodu listesi, API sözleşmesi ve yetki modeli hâlâ needs-review. |
| OD-004 | data-vault-user-data.md | Risk dağılımı / segmentasyon | Ele geçirmede tüm sırlar tek yerde açığa çıkmaması için şifreleme ve segmentasyon modeli ne? | high | decision-draft | Karar taslağı: [`vault-secret-token-decision.md`](vault-secret-token-decision.md); kısmi netleşti — tüm sırlar tek yerde açığa çıkmamalı, risk dağıtılmalı. Somut segmentasyon ve izolasyon modeli hâlâ needs-review. |
| OD-005 | data-vault-user-data.md | Şifreleme ve anahtar yönetimi | Vault içi şifreleme ve anahtar döngüsü nasıl tanımlanacak? | high | decision-draft | Karar taslağı: [`vault-secret-token-decision.md`](vault-secret-token-decision.md); kısmi netleşti — vault içinde şifreleme ve anahtar yönetimi gerekli. Algoritma, KDF, anahtar döngüsü ve HSM/secure enclave yolu hâlâ needs-review. |
| OD-006 | internal-agent-layers.md | Bando katman varlığı | Bando ayrı katman olarak kalacak mı; görev ve yetki sınırları net mi? | high | decision-draft | Karar taslağı: [`internal-communication-bando-decision.md`](internal-communication-bando-decision.md); firm ilke — Bando varsa sıradan görev ajanı değil, güvenlik/gözlem/anomali katmanı olur. Dağıtım modeli, operasyonel sınırlar ve edge-case örnekleri hâlâ needs-review. |
| OD-007 | internal-agent-layers.md | İç iletişim protokolü | Lumos→iç katman imzalama/şifreleme protokolü ve anahtar döngüsü ne? | high | decision-draft | Karar taslağı: [`internal-communication-bando-decision.md`](internal-communication-bando-decision.md); firm ilke — Lumos→iç katman iletişimi doğrulanmalı, imza/şifreleme tercih edilmeli. Protokol, mesaj formatı, anahtar döngüsü ve vault entegrasyonu hâlâ needs-review. |
...
| OD-011 | commercial-domain-payments.md | Ödeme sistemi kapsamı | Şirket yapısı netleşene kadar ödeme/PSP tamamen dışarıda mı kalacak? | high | decision-draft | Karar taslağı: [`payment-scope-decision.md`](payment-scope-decision.md); firm ilke — şirket/resmî yapı netleşene kadar ödeme, PSP, banka, merchant ve checkout kapsam dışı. Hukuk/mali çerçeve, PSP modeli ve uygulama spesifikasyonu hâlâ needs-review. |
| OD-012 | external-integrations-permissions.md | Computer Use kapısı | OpenAI Computer Use onaysız dış yazma riskine karşı hangi onay katmanı uygulanacak? | high | decision-draft | Karar taslağı: [`computer-use-permission-gate-decision.md`](computer-use-permission-gate-decision.md); firm ilke — Computer Use serbest yetkili katman değildir; Lumos geçidi, görev kapsamı, mod ayrımı ve dış etkili aksiyonlarda açık kullanıcı onayı zorunlu. Sağlayıcı, sandbox, onay UX'i, oturum süresi, log/kanıt politikası ve credential entegrasyonu hâlâ needs-review. |
...
| OD-027 | project-map-runtime-entrypoints.md | packages/kando_* geçişi | `packages/kando_*` → `src/` geçiş takvimi ve kesme kriterleri ne? | high | decision-draft | Karar taslağı: [`kando-packages-transition-decision.md`](kando-packages-transition-decision.md); kısmi netleşti — `src/` canlı, `packages/kando_*` aday/ayrılmış mimari alan. Hedef mimari, cutover kriterleri ve uygulama takvimi hâlâ needs-review. |
