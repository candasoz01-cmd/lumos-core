...
class GitHubRestApi:
    ...
    def _request_json(self, path: str, access_token: str) -> object:
        ...
        try:
            with urlopen(request, timeout=self._timeout) as response:  # noqa: S310 — sabit API
...
class GitHubReadOnlyConnector:
    ...
    def assess_write_intent(
        ...
        *,
        ...
        """Her GitHub yazma niyeti onay kapısına gider; bu bağ salt-okunurdur."""
        return self.assess_access(
            ...
            required_scopes=(
                ...
                else frozenset({GITHUB_SCOPE_REPOSITORY_METADATA_READ})
            ),
            consequential=True,
            now=now,
        )
