...
| DL-E08 | 2026-09-10 | Agent Wall görev rolleri | Sabit «Claude denetçi / Cursor uygulayıcı» yok. Roller görev bazlı: merkezî koordinasyon + değişken uygulayıcı + bağımsız son denetim. Tek yazar; teslim kaydı (dosya/dal/SHA/diff/test); self-approve yok; kilit TTL/heartbeat. **Karar onaylı, kod yazılmadı, STOP LIST sürüyor** | OD-064, [ADR-019](decisions/ADR-019-product-surface-separation-modelregistry.md), [`task-claim-v1.md`](contracts/task-claim-v1.md), [`ROADMAP.md`](ROADMAP.md) |
