...
| ID | Kaynak dosya | Konu | Kısa karar sorusu | Öncelik | Durum | Not |
|----|--------------|------|-------------------|---------|--------|-----|
| OD-060 | ADR-015 / lumos-institutional-service-foundation.md | Ticari birimler, Lumos Dünya ve ülke entegrasyonu | Lumos Bank, Lumos Sepet, Lumos POS, Lumos Dünya ve Ülke Sistemleri Entegrasyon Katmanı hangi güven sözleşmelerini paylaşır; hangi lisans, veri ve yetki kapılarıyla ayrılır? | high | decision-approved / implementation-pending | Foundation kararı: [`ADR-015`](../decisions/ADR-015-regulated-service-entity-boundaries.md). `candasoz01-cmd/Lumos` #102/#103 hizası: **Lumos Devlet kaldırıldı**, Lumos Dünya küresel yüzey; ülke bağlantısı markasız teknik katman. Entegrasyon varsayılanı: mevcut sistem authoritative, yetki `deny`, önce envanter + salt-okuma adaptörü; yazma/müdahale yalnız ülkenin sözleşmeli talebi ve operasyon bazlı matrisiyle. Kanıt skoru ve ülke paketi: [`lumos-institutional-service-foundation.md`](../analysis/lumos-institutional-service-foundation.md). **Bekleyen:** ilk ülke/sistem envanteri, veri sınıflandırma, threat model, partner ve pilot paketi. Gerçek ödeme/banka/PSP/ülke sistemi bağlantısı yok; OD-011 üst sınırı sürer. |
