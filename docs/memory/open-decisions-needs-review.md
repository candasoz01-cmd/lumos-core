...
| OD-062 | ADR-019 / ADR-004 | Ürün yüzü ayrımı ve ModelRegistry | Son kullanıcı yüzeyi ile iç operatör yüzeyi (Command Wall) nasıl ayrılır; model/provider seçimi Router'ın altında hangi katmanda durur? | high | **decision-approved / implementation-pending** | Karar: [`ADR-019`](../decisions/ADR-019-product-surface-separation-modelregistry.md) · [`ADR-004`](../decisions/ADR-004-ai-router-routing-layer.md) § Router altında ModelRegistry sınırı. Hedef: `Lumos User Surface → Router → ModelRegistry → Providers`, ayrı gözlem yüzeyi `Command Wall → Router + Agent/Session/Task state`. `ModelRegistry` ≠ `IntegrationRegistry` normatif. Kullanıcı yalnız Lumos görür (PR-005). ADR-008 öncelik sırası (Firewall → Trust → Router → Memory → Agent Network) değişmedi. **Bekleyen:** `ModelRegistry` arayüz sözleşmesi; Router model seçim sinyalleri; Command Wall operatör yüzeyi teknik paketi + yetkilendirme; kullanıcı yüzü sızıntı guard testi — kapsam kabul kriteri ADR-019 § Bekleyen'de tanımlı (`provider`, `agent_id`, `instance_id`, `session_id`, `workspace_path`, heartbeat, PR/merge gate; public API yanıtları dahil); FAZ-1 sonrası provider evaluation paketi (Kimi/Moonshot backlog'da). **Kod yazılmadı; STOP LIST sürüyor.** |
