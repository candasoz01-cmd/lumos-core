...
def test_scope_expansion_and_write_intent_require_approval_without_vault_read(tmp_path):
    ...
    write_intent = connector.assess_access(
        owner_id=OWNER_ID,
        account_id=ACCOUNT_ID,
...
def test_infisical_v4_write_sends_token_but_never_returns_it(mock_urlopen):
    ...
    assert result.ok is True
    assert request.full_url == "https://vault.test/api/v4/secrets/github-opaque-ref"
...
def test_github_http_error_never_exposes_token(mock_urlopen):
    ...
    try:
        ...
        raise AssertionError("GitHubApiError expected")
