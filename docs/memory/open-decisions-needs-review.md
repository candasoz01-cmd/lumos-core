...
|----|--------------|------|-------------------|---------|--------|-----|
| OD-001 | security-architecture.md | Lumos Vault uygulaması | Secret'lar hangi vault/katman modelinde tutulacak; Lumos yüzeyinden nasıl ayrılacak? | high | decision-approved / implementation-pending | Karar: [`vault-secret-token-decision.md`](vault-secret-token-decision.md) — Lumos secret taşımaz; vault/kasa ayrı güvenli katman; Lumos yetkili geçit/orkestratör. **V1 teknoloji seçimi:** [`od-vault-v1-technology-selection.md`](od-vault-v1-technology-selection.md) — **harman + OSS self-host çekirdek** (sıfırdan reddedildi); tam aday/değerlendirme **private strategy vault**. Yol haritası: [`vault-implementation-roadmap.md`](vault-implementation-roadmap.md). **Bekleyen:** PoC, depolama/dağıtım (V2), bridge adapter (**private**). |
...
Son güncelleme: 2026-06-21 (OD-031 Phase 2 Step 2 — OD-001 vault tech redaction)
