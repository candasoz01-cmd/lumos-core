...
async def run_telegram_listener(
    cfg: TgConfig, *, db_path: str | Path, bus: EventBus
) -> int:
    ...
    if not sources:
        print('no tg sources. add one: python -m lumos_social tg follow "@username"')
        return 0
    ...
    async def handler(event: events.NewMessage.Event) -> None:
        try:
            ...
            bus.emit(Event(type="telegram.message", payload=payload))
        except Exception as e:
            bus.emit(Event(type="telegram.error", payload={"error": str(e)}))
