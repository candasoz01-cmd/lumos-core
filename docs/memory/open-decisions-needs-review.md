...
| OD-046 | project-map-runtime-entrypoints.md | Root build vs panel E2E | `npm run build` (ui) ile panel E2E hangi yüzeyi hedefler? | medium | decision-approved / **implementation-partial** | Karar: [`build-e2e-surface-alignment-decision.md`](build-e2e-surface-alignment-decision.md); Seçenek A onaylandı — üretim `ui/`, kök legacy E2E `panel/`. **v1:** `e2e:smoke:ui` + `ui/e2e/smoke-panel.mjs` (PR #294). **v2:** CI `ui-smoke` job — `ui` build + Playwright smoke. Tam E2E migrasyonu bekliyor. |
...
Son güncelleme: 2026-06-20 (OD-046 v2 CI smoke; implementation-partial; DL-F07 sync)
