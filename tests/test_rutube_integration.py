...
def test_rutube_oauth_contract_uses_env_and_reports_identity_required(monkeypatch):
    for name in (
...
def test_rutube_publish_requires_explicit_approval_and_never_fakes_execution():
    denied = _run("publish")
    ...
    assert approved.error == "rutube_publish_connector_not_live"
